...
RUN mkdir -p /usr/local/src \
  ...
  && time env PYTORCH_BUILD_VERSION=${CoreAI_TORCH} PYTORCH_BUILD_NUMBER=0 USE_CUDA=0 USE_CUDNN=0 USE_MKLDNN=1 USE_FFMPEG=1 USE_OPENCV=1 python3 ./setup.py bdist_wheel | tee /tmp/torch_config.txt \
...
RUN mkdir -p /usr/local/src \
  ...
  && time env BUILD_VERSION=${CoreAI_TORCHVISION} FORCE_CUDA=0 TORCHVISION_USE_FFMPEG=1 python3 ./setup.py bdist_wheel | tee /tmp/torchvision_config.txt \
...
RUN mkdir -p /usr/local/src \
  ...
  && time env BUILD_VERSION=${CoreAI_TORCHAUDIO} USE_CUDA=0 USE_FFMPEG=1 python3 ./setup.py bdist_wheel | tee /tmp/torchaudio_config.txt \
